...
package de.kaiserpfalzedv.billing.api.client;

import de.kaiserpfalzedv.billing.api.mapper.ApiBillingClientException;
import de.kaiserpfalzedv.billing.api.mapper.ResponseErrorMapper;
import de.kaiserpfalzedv.billing.api.model.client.Client;
...
/**
 * <p>ClientClient -- Client for accessing the assets of a library.</p>
 *
...
public interface ClientClient {
    String CACHE_NAME = "api-billing-client";
    ...
    @Retry(
            delay = 2000,
            maxDuration = 6000,
            retryOn = {ApiBillingClientException.class}
    )
    @CircuitBreaker(
            failOn = ApiBillingClientException.class,
            requestVolumeThreshold = 5
    )
    @POST
    @CacheResult(cacheName = CACHE_NAME, lockTimeout = CACHE_LOCK_TIMEOUT)
    Client create(final Client data);
    ...
    @Path("/{id}")
    @CacheResult(cacheName = CACHE_NAME, lockTimeout = CACHE_LOCK_TIMEOUT)
    Client retrieve(@PathParam("id") final UUID id);
    ...
    void update(@PathParam("id") final UUID id, final Client asset);
